...
export function WeightingBasis({ project, module }: { project: Project; module: ModuleKey }) {
  ...
  const save = () => {
    updateProject(project.id, {
      progressWeights: {
        ...(project.progressWeights ?? {}),
        [module]: draft,
      } as Project["progressWeights"],
    });
    setEditing(false);
  };
            ...
            <>
              {isOverridden && (
                <button
                  ...
                  className="inline-flex items-center gap-1 rounded-md border border-border px-2 h-7 text-[11px] font-semibold hover:bg-muted/50"
                >
              ...
              <button
                onClick={startEdit}
                className="inline-flex items-center gap-1 rounded-md bg-secondary px-2 h-7 text-[11px] font-semibold hover:bg-secondary/80"
              >
              ...
              <div className="mt-1.5 h-1.5 rounded-full bg-muted overflow-hidden">
                <div
                  className={cn(
                    ...
                    isOverridden && !editing ? "bg-warning" : "bg-primary",
                  )}
          ...
          <div className="text-[11px] text-muted-foreground">
            ...
            <span
              className={cn(
                ...
                draftTotal === 0 ? "text-destructive" : "text-foreground",
              )}
            >
              {draftTotal}
            </span>
            <span className="ml-2 text-muted-foreground/70">
              — values normalise automatically to 100%.
            </span>
          </div>
          <div className="flex gap-1.5">
            <button
              onClick={() => setEditing(false)}
              className="inline-flex items-center gap-1 rounded-md border border-border px-3 h-8 text-xs font-semibold hover:bg-muted/50"
            >
            ...
            <button
              ...
              className="inline-flex items-center gap-1 rounded-md bg-primary px-3 h-8 text-xs font-semibold text-primary-foreground hover:bg-primary/90 disabled:opacity-40"
            >
