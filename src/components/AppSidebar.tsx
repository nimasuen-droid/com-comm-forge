...
export function SidebarBody() {
  ...
  const projectExists = useStore((s) =>
    activeId ? s.projects.some((p) => p.id === activeId) : false,
  );
  ...
  const projectNav =
    projectExists && activeId
      ? [
          {
            to: "/projects/$projectId/systems" as const,
            slug: "systems",
            label: "Systemization",
            icon: Network,
          },
          {
            to: "/projects/$projectId/preservation" as const,
            slug: "preservation",
            label: "Preservation",
            icon: Wrench,
          },
          {
            to: "/projects/$projectId/punch" as const,
            slug: "punch",
            label: "Punch List",
            icon: ListChecks,
          },
          {
            to: "/projects/$projectId/mc" as const,
            slug: "mc",
            label: "Mechanical Completion",
            icon: ShieldCheck,
          },
          {
            to: "/projects/$projectId/commissioning" as const,
            slug: "commissioning",
            label: "Commissioning",
            icon: Activity,
          },
          {
            to: "/projects/$projectId/turnover" as const,
            slug: "turnover",
          ...
          {
            to: "/projects/$projectId/workflow" as const,
            slug: "workflow",
        ...
        <Link to="/" className="flex items-center gap-2">
          ...
          <div>
            ...
            <div className="text-sm font-bold leading-tight">
              Commissioning <span className="text-accent">Pro</span>
            </div>
          </div>
        </Link>
      </div>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      <div className="px-2 mb-2 text-[10px] font-mono uppercase tracking-widest text-muted-foreground">
        {title}
      </div>
