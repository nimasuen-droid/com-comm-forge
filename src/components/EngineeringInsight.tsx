...
export function EngineeringInsight({ title = "Engineering Insight", why, problems, best, defaultOpen = false, variant = "default" }: Props) {
    ...
    <div className={cn("panel overflow-hidden", variant === "compact" && "text-sm")}>
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center gap-3 px-4 py-3 text-left hover:bg-muted/30 transition-colors">
        ...
        <div className="flex-1">
          ...
          <div className="text-xs text-muted-foreground">Why it matters · Common field issues · Best practice</div>
        </div>
      ...
      {open && (
        <div className="px-4 pb-4 pt-1 grid md:grid-cols-3 gap-3 border-t border-border">
          ...
          <Block icon={<AlertTriangle className="h-4 w-4 text-warning" />} label="Common field problems" tone="warning">
            {problems}
          </Block>
          <Block icon={<CheckCircle2 className="h-4 w-4 text-success" />} label="Best practice" tone="success">
            {best}
          </Block>
        </div>
      )}
...
function Block({ icon, label, children, tone }: { icon: ReactNode; label: string; children: ReactNode; tone: "info" | "warning" | "success" }) {
  const border = tone === "info" ? "border-info/30" : tone === "warning" ? "border-warning/30" : "border-success/30";
  return (
