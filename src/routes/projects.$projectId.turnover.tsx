  ...
  return (
    <div className="space-y-5">
        ...
        <table className="w-full text-sm">
          <thead className="bg-muted/30 text-[10px] font-mono uppercase tracking-wider text-muted-foreground">
            <tr>
              ...
              {TURNOVER_CHECK_KEYS.map((k, i) => <th key={k} className="px-2 py-3 text-center">{i+1}. {TURNOVER_CHECK_LABELS[k]}</th>)}
              ...
              return (
                <tr key={ss.id} className="hover:bg-muted/20">
                      ...
                      <td key={k} className="px-2 py-3 text-center">
                        <button
                          ...
                          className={cn(
                            ...
                            checked ? "bg-accent border-accent text-accent-foreground" : "border-border bg-background hover:bg-muted/40"
                          )}
                        >
                  ...
                  <td className="px-2 py-3 text-center">
                    <span className={cn("inline-block px-2 py-0.5 rounded text-xs font-bold", ragColor[ss.turnoverStatus])}>{ss.turnoverStatus}</span>
                  </td>
                </tr>
              );
            }))}
      ...
      <EngineeringInsight
        ...
        defaultOpen
        why={<>Operations cannot accept the entire plant in one go. Phased turnover transfers <b>care, custody &amp; control</b> subsystem by subsystem. Each tick is a contractual gate.</>}
        problems={<>Dossiers incomplete (missing as-builts, vendor manuals); operations refuses to sign because punches not reconciled; ambiguity over who maintains preservation post-MC.</>}
        best={<>Build the dossier as you go — never at the end; require operations witnessing from MC walkdowns; export the Handover Dossier and walk it system-by-system with the asset team.</>}
      />
