...
import { Activity, AlertTriangle, ListChecks, Package, TrendingUp, Zap, ArrowRight, Gauge } from "lucide-react";

export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "Portfolio Dashboard — Completions & Commissioning Pro" },
      { name: "description", content: "Multi-project EPC completions and commissioning portfolio dashboard." },
    ],
  }),
  component: Dashboard,
});
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-end justify-between flex-wrap gap-3">
        <div>
          <div className="text-[11px] font-mono uppercase tracking-widest text-muted-foreground">Portfolio Overview</div>
          <h1 className="text-3xl font-bold mt-1">Completions Command Center</h1>
          <p className="text-muted-foreground text-sm mt-1">Live readiness, punch posture, and commissioning sequence across {projects.length} active project{projects.length === 1 ? "" : "s"}.</p>
        </div>
        <Link to="/projects" className="inline-flex items-center gap-2 rounded-md bg-accent text-accent-foreground px-4 py-2 text-sm font-semibold hover:bg-accent/90">
          Manage Projects <ArrowRight className="h-4 w-4" />
        </Link>
      </div>

      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <KpiTile icon={<Package />} label="Active Projects" value={projects.length} accent="primary" />
        <KpiTile icon={<Gauge />} label="Avg MC Readiness" value={`${Math.round(totals.mc/n)}%`} accent="success" />
        <KpiTile icon={<Activity />} label="Avg Commissioning" value={`${Math.round(totals.comm/n)}%`} accent="info" />
        <KpiTile icon={<AlertTriangle />} label="Open A-Punches" value={totals.openA} accent="destructive" sub={`${totals.openB} B · ${totals.openC} C`} />
      </div>

      <div className="grid lg:grid-cols-3 gap-5">
        <div className="lg:col-span-2 space-y-3">
          <h2 className="text-sm font-mono uppercase tracking-widest text-muted-foreground px-1">Project Portfolio</h2>
          {projects.length === 0 ? (
            <div className="panel p-10 text-center">
              <p className="text-muted-foreground">No active projects. Create one to get started.</p>
              <Link to="/projects" className="inline-block mt-4 rounded-md bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground">New Project</Link>
            </div>
          ) : projects.map(p => {
            const k = projectKpis(p);
            return (
              <Link key={p.id} to="/projects/$projectId/systems" params={{ projectId: p.id }} onClick={() => setActive(p.id)}
                className="panel block p-5 hover:border-primary/50 transition-colors group">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <div className="text-[10px] font-mono uppercase tracking-widest text-muted-foreground">{p.client} · {p.location}</div>
                    <div className="text-lg font-bold mt-0.5 group-hover:text-primary transition-colors">{p.name}</div>
                    <div className="text-xs text-muted-foreground mt-1">{p.type}</div>
                  </div>
                  <div className="flex gap-1.5 flex-wrap justify-end">
                    {k.punchA > 0 && <span className="tag-chip !bg-destructive/20 !border-destructive/40 text-destructive">A:{k.punchA}</span>}
                    {k.punchB > 0 && <span className="tag-chip !bg-warning/20 !border-warning/40 text-warning">B:{k.punchB}</span>}
                ...
                <div className="mt-4 flex items-center gap-4 text-xs text-muted-foreground font-mono">
                  ...
                  <span>·</span>
                  <span>{k.punchOpen}/{k.punchTotal} punches open</span>
                </div>
              </Link>
            );
          })}
        ...
        <aside className="space-y-4">
          <div className="panel p-5">
            ...
            <ul className="space-y-2.5 text-sm">
              {projects.flatMap(p => p.punches.filter(x => x.category === "A" && x.status !== "closed").slice(0, 4).map(x => (
                <li key={x.id} className="flex items-start gap-2 border-l-2 border-destructive pl-3 py-0.5">
              ...
              {projects.every(p => !p.punches.some(x => x.category === "A" && x.status !== "closed")) && (
...
function KpiTile({ icon, label, value, accent, sub }: { icon: React.ReactNode; label: string; value: React.ReactNode; accent: string; sub?: string }) {
  ...
  return (
    <div className="stat-tile relative overflow-hidden">
      <div className={`absolute -right-6 -top-6 h-24 w-24 rounded-full bg-gradient-to-br ${colors[accent]} blur-xl opacity-40`} />
      <div className="relative">
        <div className="flex items-center justify-between">
          <div className="text-[10px] font-mono uppercase tracking-widest text-muted-foreground">{label}</div>
          <div className={`h-8 w-8 rounded-md border flex items-center justify-center ${colors[accent].split(" ").slice(-1)[0]}`}>
