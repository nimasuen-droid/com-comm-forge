  ...
  return (
    <div className="space-y-5">
            ...
            <tr>
              ...
              {COMM_CHECK_KEYS.map((k, i) => (
                <th key={k} className="px-2 py-3 text-center">
                  {i + 1}. {COMM_CHECK_LABELS[k].split(" ")[0]}
                </th>
              ))}
      ...
      <EngineeringInsight
        ...
        why={
          <>
            Each tick advances commissioning RAG automatically. Sequence (utilities first, then
            process) determines whether commissioning takes weeks or months.
          </>
        }
        problems={
          <>
            Schedule pressure pushes process loop checks before ICSS is stable; vendors arrive after
            their package is needed; reliability runs start with open A-punches.
          </>
        }
        best={
          <>
            Lock energisation order with operations and SIMOPS team; align vendor mobilisation to
            readiness; never enter reliability run with open A-punches.
          </>
        }
      />
      ...
      <WorkflowNav
        ...
        next={{
          to: "/projects/$projectId/turnover",
          projectId: project.id,
