...
function PresPage() {
  ...
  const updateSub = (
    sysId: string,
    subId: string,
    patch: { interval?: number; lastDone?: string | undefined },
  ) => {
  ...
  return (
    <div className="space-y-5">
      <LearnRail module="preservation" title="Learn: Preservation" />
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <h2 className="text-xl font-bold flex items-center gap-2">
          <Wrench className="h-5 w-5 text-info" /> Preservation Register
        </h2>
        <button
          onClick={() => exportPreservation(project)}
                ...
                <tr key={ss.id} className="hover:bg-muted/20">
                  ...
                  <td className="px-2 py-3">
                    <input
                      ...
                      value={interval}
                      onChange={(e) =>
                        updateSub(sys.id, ss.id, { interval: Number(e.target.value) })
                      }
                  ...
                  <td className="px-2 py-3 text-xs">
                    <input
                      type="date"
                      value={ss.preservation?.lastDone?.slice(0, 10) ?? ""}
                      onChange={(e) =>
                        updateSub(sys.id, ss.id, {
                          interval,
      ...
      <EngineeringInsight
        ...
        why={
          <>
            Equipment in lay-up degrades — bearings flat-spot, motor windings absorb moisture,
            instrument transmitters drift, piping internals corrode. Failures appear at first run,
            weeks after MC, with no clear owner.
          </>
        }
        problems={
          <>
            Logs stop after MC; rotating equipment not turned monthly; N₂ blanket pressure not
            maintained; battery rooms left without trickle charge.
          </>
        }
        best={
          <>
            Treat preservation as a live register from delivery to start-up; assign owner per
            subsystem; audit monthly with operations; re-baseline preservation when systems change
            custody.
          </>
        }
      />
